...
class MainWindow(QMainWindow):
    ...
    def data_changed(self):
        ...
        if self.model.data:  # toggle if specific conditions are met
            ...
            self.all_actions["save_pipeline"].setEnabled(
                enabled and self.model.current.get("parent_index") is not None
            )
    ...
    def show_history(self):
        """Show history."""
        HistoryDialog(self, self.model.get_history(), self.model.log).exec()
    ...
    def save_pipeline(self):
        ...
        if fname:
            if not fname.endswith(".mnepipe"):
                fname += ".mnepipe"
            self.model.save_pipeline(path=fname)
    ...
    def apply_pipeline_from_file(self):
        ...
        if fname:
            try:
                ...
                return
            dialog = ApplyPipelineDialog(self, pipeline, self.model.current)
            if dialog.exec():
                try:
                    self.model.apply_pipeline(pipeline)
