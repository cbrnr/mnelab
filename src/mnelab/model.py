...
class Model:
    ...
    @data_changed
    def annotations_from_event_intervals(self, interval_data):
        """Create annotations between events."""
        fs = self.current["data"].info["sfreq"]
        all_events = self.current["events"]

        start_event_id = int(interval_data["start_event"])
        end_event_id = int(interval_data["end_event"])

        starts_raw = all_events[all_events[:, 2] == start_event_id, 0] / fs
        ends_raw = all_events[all_events[:, 2] == end_event_id, 0] / fs
        ...
        if valid_onsets:
            self.current["data"].annotations.append(
                valid_onsets,
                valid_durations,
        ...
        if interval_data["extend_start"]:
            ...
            if first_boundary > 0:
                self.current["data"].annotations.append(
                    0.0, first_boundary, interval_data["annotation"]
                )

        if interval_data["extend_end"]:
            last_samp_time = self.current["data"].last_samp / fs
            boundaries = []
            if len(starts_raw) > 0:
            ...
            if duration > 0:
                self.current["data"].annotations.append(
                    last_boundary, duration, interval_data["annotation"]
                )
