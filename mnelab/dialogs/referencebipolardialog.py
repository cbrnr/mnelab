...
from qtpy.QtWidgets import QDialog, QVBoxLayout, QListWidget, QDialogButtonBox, QPushButton, QLabel, QGridLayout, QGroupBox, QFileDialog
...
class ReferenceBipolarDialog(QDialog):
    ...
    def add_buttons(self):
        ...
        if(len(self.channels.selectedItems()) == 2 ):
            pair = [item.data(0) for item in self.channels.selectedItems()] # pair list
    ...
    def save_buttons(self):
        ...
        with open(fname,'w') as file:
            sel = [self.selected.item(i).text() for i in range(self.selected.count())]
            sell = [[ i.strip("'").strip('"') for i in j[1:-1].split(", ")] for j in sel]
